...
//	case F_GOLOGICALLINEEND_BOX:Sub_BoxSelectLock(lparam2); this->Command_GOLINEEND( true, 0, 8 | lparam1 );break;	//(矩形選択)行末に移動(改行単位)
	...
	/* 検索系 */
	case F_SEARCH_DIALOG:		Command_SEARCH_DIALOG(( int )lparam1 );break;							//検索(単語検索ダイアログ)
	...
	case F_REPLACE_DIALOG:	//置換(置換ダイアログ)
		Command_REPLACE_DIALOG(( int )lparam1 );	//@@@ 2002.2.2 YAZAKI ダイアログ呼び出しと、実行を分離
		break;
	...
	case F_GREP_REPLACE_DLG:	//Grep置換ダイアログの表示
		...
		Command_GREP_REPLACE_DLG(nCommand == F_GREP_REPLACE_DLG);
		return bRet;
	case F_GREP_OR_REPLACE:	Command_GREP_REPLACE();break;						//Grep検索・置換共用
