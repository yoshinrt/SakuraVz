...
/*! GREP置換ダイアログの表示
*/
void CViewCommander::Command_GREP_REPLACE_DLG( bool bReplace )
{
	...
	int nRet = cDlgGrepRep.DoModal( bReplace, G_AppInstance(), m_pCommanderView->GetHwnd(), GetDocument()->m_cDocFile.GetFilePath(), (LPARAM)m_pCommanderView );
	if( !nRet ){
		return;
	}
	HandleCommand(F_GREP_OR_REPLACE, TRUE, 0, 0, 0, 0);	//	GREPコマンドの発行
}
...
void CViewCommander::Command_GREP_REPLACE( void )
{
	...
	*/
	...
	// 編集ウィンドウの上限チェック
	else if( GetDllShareData().m_sNodes.m_nEditArrNum >= MAX_EDITWINDOWS ){	//最大値修正	//@@@ 2003.05.31 MIK
		OkMessage( m_pCommanderView->GetHwnd(), LS(STR_MAXWINDOW), MAX_EDITWINDOWS );
		return;
	}
	
	else if(cDlgGrepRep.m_bReplace){
		/*======= Replaceの実行 =============*/
		/* Grep結果ウィンドウの表示 */
		cmWork1.Replace( L"\"", L"\"\"" );
		cmWork2.Replace( L"\"", L"\"\"" );
		...
		/*======= Grepの実行 =============*/
		/* Grep結果ウィンドウの表示 */
		CControlTray::DoGrepCreateWindow(G_AppInstance(), m_pCommanderView->GetHwnd(), cDlgGrepRep);
	}
	return;
}
