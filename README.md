...
- [Grep検索・Grep置換のマルチスレッド化](#grep検索grep置換のマルチスレッド化)
...
### 改行をまたぐ検索・置換
正規表現で改行をまたぐ検索・置換に対応しました．ただし以下の制限があります．
...
- 本家の並列処理に加え，追加で並列処理を導入する等の高速化を行いました．
- 巨大なファイル (10MB 以上，設定可能) で一部の機能を制限することにより，更に高速化する「高速モード」を追加しました．高速モードでは，以下の動作に制限が入ります．
  - 折り返し設定は 5120文字で固定され，他の折り返し設定は使用できません．全角・半角にかかわらず 5120文字なので，全角・半角が混在する文書では折り返し位置が揃わないことになります．
  - ブロックコメントや文字列リテラル等の一部のカラーリングが，ごく稀に不正確になります．

1GB のファイルオープンに要する時間 (秒) は以下のとおりです．(使用 PC: AMD Ryzen5 5600G (12スレッド)  Mem:16GB)

|折返し設定|[Sakura本家 v2.4.2.6206](https://github.com/sakura-editor/sakura/tree/cf66c279711ed035cc0253b87618714cca9fbb50)|SakuraVz (非高速モード)|SakuraVz (高速モード)|
|------------|---:|---:|---:|
|折返さない  |13.6|5.2|3.7|
|右端で折返す|24.3|9.1|3.7<br>(強制的に折返さない)|
...
### Grep検索・Grep置換のマルチスレッド化

Grep検索・Grep置換 をマルチスレッド化することにより，高速化しました．

サクラエディタの GitHub リポジトリで 'CNative' を検索時の時間 (秒) は以下のとおりです．(使用 PC: AMD Ryzen5 5600G (12スレッド)  Mem:16GB)

|[Sakura本家 v2.4.2.6109](https://github.com/sakura-editor/sakura/tree/7eb997a00e73d2cbdd071d226694c0b55f8925bf)|SakuraVz|
|---:|---:|
|4.7|1.1|
